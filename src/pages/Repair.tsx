...
import { Wrench, Smartphone, Battery, Droplets, Bug, Shield, Clock, Leaf, Truck, Cpu, Award } from 'lucide-react';
...
const Repair = () => {
  ...
  const [formData, setFormData] = useState({ name: '', phone: '', device: '', issue: '', urgency: 'Normal' });

  const repairServices = [
    { icon: Smartphone, title: 'Screen Replacement', description: 'Cracked or damaged screens repaired with genuine parts', price: 'Starting from ₹1,999', time: '30-60 mins' },
    { icon: Battery, title: 'Battery Replacement', description: 'Original battery replacement for longer device life', price: 'Starting from ₹1,499', time: '20-30 mins' },
    { icon: Droplets, title: 'Water Damage Repair', description: 'Professional water damage recovery services', price: 'Starting from ₹2,999', time: '2-4 hours' },
    { icon: Bug, title: 'Software Issues', description: 'OS installation, virus removal, performance optimization', price: 'Starting from ₹799', time: '30-90 mins' },
    { icon: Wrench, title: 'Charging Port Repair', description: 'Fix loose or damaged charging ports', price: 'Starting from ₹1,299', time: '45-60 mins' },
    { icon: Shield, title: 'Camera Repair', description: 'Front and rear camera repair services', price: 'Starting from ₹1,799', time: '60-90 mins' }
  ];

  const features = [
    { icon: Cpu, title: 'AI-Powered Diagnostics', description: 'E-Genie™ AI Assistant for instant device diagnostics' },
    { icon: Truck, title: 'Doorstep Pickup & Delivery', description: 'Mobile van service for convenient on-the-go repairs' },
    { icon: Leaf, title: 'Eco-Friendly Service', description: 'Sustainable practices with responsible e-waste recycling' },
    { icon: Clock, title: 'Same-Day Repairs', description: 'Most common issues fixed within hours' },
    { icon: Shield, title: 'Up to 6-Month Warranty', description: 'Genuine spare parts with extended warranty coverage' },
    { icon: Award, title: 'Certified Technicians', description: 'Expert repair for all major brands and models' }
  ];

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const validationResult = repairSchema.safeParse({
      name: formData.name.trim(), phone: formData.phone.trim(), device: formData.device.trim(),
      issue: formData.issue.trim(), urgency: formData.urgency.toLowerCase() as 'normal' | 'urgent' | 'emergency'
    });
    ...
    const { error } = await supabase.from('repair_requests').insert({
      name: validatedData.name, phone: validatedData.phone, device: validatedData.device,
      issue: validatedData.issue, urgency: validatedData.urgency
    });
    if (error) {
      if (import.meta.env.DEV) console.error('Error saving repair request:', error);
      toast({ title: "Submission failed", description: "Unable to process your request. Please try again.", variant: "destructive" });
      return;
    }
    const message = encodeURIComponent(`New Repair Booking:\nName: ${validatedData.name}\nPhone: ${validatedData.phone}\nDevice: ${validatedData.device}\nIssue: ${validatedData.issue}\nUrgency: ${validatedData.urgency}`);
    window.open(`https://wa.me/918999895516?text=${message}`, '_blank');
    toast({ title: "Repair request sent!", description: "We'll contact you shortly to confirm your booking." });
    setFormData({ name: '', phone: '', device: '', issue: '', urgency: 'Normal' });
  };
